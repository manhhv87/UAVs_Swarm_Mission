...
def keep_sending_fly_follow_command(control_word):
    ...
    while True:
        ...
        if (control_word_value == 'T'):  # T = Terminate
            ...
            follower1_command_str = 'fly_follow({}, {}, {}, {}, {})'.format(
            ...
            follower2_command_str = 'fly_follow({}, {}, {}, {}, {})'.format(
            ...
            follower3_command_str = 'fly_follow({}, {}, {}, {}, {})'.format(
...
THREAD_keep_sending_fly_follow_command = threading.Thread(
    target=keep_sending_fly_follow_command, args=(control_word,))
THREAD_keep_sending_fly_follow_command.start()

# Create action queue.
action_queue = Queue.Queue()
# Start thread execute_function_in_queue().
THREAD_execute_function_in_queue = threading.Thread(
    target=execute_function_in_queue, args=(action_queue,))
...
# Run keep_sending_fly_follow_command().
action_queue.put((control_word.set_run,))
...
# Terminate the thread keep_sending_fly_follow_command().
action_queue.put((control_word.set_terminate,))
action_queue.put((air_break,))  # Stop leader.
# 'End' must be in the end. Terminate execute_function_in_queue thread.
action_queue.put(('End',))

...
follower1_path_to_save_frames = '/home/iris2/saved_frames/{:%Y%m%d_%H-%M-%S}'.format(
    datetime.now()) + '/'
follower2_path_to_save_frames = '/home/iris3/saved_frames/{:%Y%m%d_%H-%M-%S}'.format(
    datetime.now()) + '/'
follower3_path_to_save_frames = '/home/iris4/saved_frames/{:%Y%m%d_%H-%M-%S}'.format(
    datetime.now()) + '/'

command_str_to_follower1 = command_str_factory.format(
    yellow_color_range_pair, time_out, max_altitude, max_radius, follower1_path_to_save_frames)
command_str_to_follower2 = command_str_factory.format(
    orange_color_range_pair, time_out, max_altitude, max_radius, follower2_path_to_save_frames)
command_str_to_follower3 = command_str_factory.format(
    pink_color_range_pair, time_out, max_altitude, max_radius, follower3_path_to_save_frames)

command_tuple = (command_str_to_follower1,
                 command_str_to_follower2, command_str_to_follower3)
...
for i_temp in xrange(len(follower_host_tuple)):
    print('{} - Sending immediate command to : {}.'.format(time.ctime(),
          follower_host_tuple[i_temp]))
    dcf.CLIENT_send_immediate_command(
        follower_host_tuple[i_temp], command_tuple[i_temp])
# Start instance.
for i_temp in xrange(len(follower_host_tuple)):
    print('{} - Sending immediate command to : {}.'.format(time.ctime(),
          follower_host_tuple[i_temp]))
    dcf.CLIENT_send_immediate_command(
        follower_host_tuple[i_temp], 'balloon_terminator.find_and_destroy_balloon()')

...
def find_shape_center_and_radius(binary_image):
    contours = cv2.findContours(
        binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    if contours:  # contours is a list. The boolean value of an empty list is False.
        # key is a function applied to contours.
...
def calculateDisparity(framePair):
    ...
    if (isSGBM):
        ...
        stereo = cv2.StereoBM_create(
            numDisparities=16*num_disp, blockSize=2*block_size+1)
...
def find_max_contour(binary_image):
    ...
    contours = cv2.findContours(binary_image.copy(
    ), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]  # cv2.CHAIN_APPROX_NONE
    if contours:  # contours is a list. The boolean value of an empty list is False.
        # key is a function applied to contours.
        max_contour = max(contours, key=cv2.contourArea)
        return max_contour
...
def extract_colored_shape(image, color_range_lower, color_range_upper, colorspace='HSV', floodFill=False):
    ...
    if (colorspace == 'HSV'):
        ...
        # Hue range crosses 0 degree.
        if (color_range_lower[0] > color_range_upper[0]):
            # t1 = time.time()
            # Split to two ranges.
    ...
    if floodFill:
        ...
        fillMask_for_mask_floodFilled = np.zeros(
            (height_mask_floodFilled+2, width_mask_floodFilled+2), np.uint8)
        # Flood fill mask_floodFilled with white color.
        cv2.floodFill(mask_floodFilled,
                      fillMask_for_mask_floodFilled, (0, 0), 255)
...
class Thread_detect_balloon(threading.Thread):
    def __init__(self, cameraL, cameraR, color_range_pair):
        ...
        self._isRunning_Thread_detect_balloon = ControlFlag_bool(
            bool_value=False)
    ...
    def run(self):  # Capture frames and save to deque.
        ...
        while self._isRunning_Thread_detect_balloon.value:
            ...
            frameL = self._cameraL.decode_MJPG_downsample(
                raw_frameL, (480, 270), color=1)
            # Decode raw jpeg image.
            frameR = self._cameraR.decode_MJPG_downsample(
                raw_frameR, (480, 270), color=1)
            ...
            if (frameL is not None) & (frameR is not None):
                ...
                foregroundL = extract_colored_shape(
                    frameL, color_range_lower, color_range_upper, colorspace='HSV')
                foregroundR = extract_colored_shape(
                    frameR, color_range_lower, color_range_upper, colorspace='HSV')
                ...
                if (max_contourL is not None) & (max_contourR is not None):
                    ...
                    (centerL_x_enclose, centerL_y_enclose), radiusL_enclose = min_enclosing_circle(
                        max_contourL)
                    (centerR_x_enclose, centerR_y_enclose), radiusR_enclose = min_enclosing_circle(
                        max_contourR)
                    # Put result into tuple.
                    enclose_circle = ((centerL_x_enclose, centerL_y_enclose, radiusL_enclose), (
                        centerR_x_enclose, centerR_y_enclose, radiusR_enclose))
                    # Find least square fitted circle.
                    (centerL_x_fit, centerL_y_fit), radiusL_fit = least_square_circle_fit(
                        max_contourL)
                    (centerR_x_fit, centerR_y_fit), radiusR_fit = least_square_circle_fit(
                        max_contourR)
                    # Put result into tuple.
                    fit_circle = ((centerL_x_fit, centerL_y_fit, radiusL_fit),
                                  (centerR_x_fit, centerR_y_fit, radiusR_fit))
                    # Put results into tuple.
                    result = ((frameL, frameR), (foregroundL,
                              foregroundR), enclose_circle, fit_circle)
                else:
    ...
    class Thread_record_video(threading.Thread):
        def __init__(self, frameQueue,
            ...
            self._isRunningThread_record_video = ControlFlag_bool(
                bool_value=False)
        ...
        def run(self):
            ...
            t1 = time.time()
            # If .value is true.
            while (self._isRunningThread_record_video.value):
                ...
                while (len(ms_str) < 3):
                    ms_str = ms_str + '0'
                time_stamp_save = '{}.{}'.format(
                    time.strftime('%Y%m%d_%H-%M-%S'), ms_str)
                time_stamp_display = '{}.{}'.format(
                    time.strftime('%Y-%m-%d %H:%M:%S'), ms_str)
                ...
                print(
                    '{} - fit (x, y, r): {}'.format(time_stamp_display, result[2]))
                print(
                    '{} - enc (x, y, r): {}'.format(time_stamp_display, result[3]))
                if self._isPlotOriginalFrame & (frameL is not None) & (frameR is not None):
                    frameL = result[0][0]
                    frameR = result[0][1]
                    frame_pair = np.hstack((frameL, frameR))
                    cv2.putText(frame_pair, time_stamp_display,
                                (18, 28), 2, 1, (0, 0, 0), 2)
                    cv2.putText(frame_pair, time_stamp_display,
                                (18, 28), 2, 1, (255, 255, 255), 1)
                    if self._displayOrSave == 'display':
                        cv2.imshow('original', frame_pair)
                    elif self._displayOrSave == 'save':  # Save frame.
                        cv2.imwrite(self._framePath + '/' +
                                    time_stamp_save+'_original.jpg', frame_pair)
                    # Display and save frame.
                    elif self._displayOrSave == 'both':
                        cv2.imshow('original', frame_pair)
                        cv2.imwrite(self._framePath + '/' +
                                    time_stamp_save+'_original.jpg', frame_pair)
                if self._isPlotForeground & (foregroundL is not None) & (foregroundR is not None):
                    ...
                    cv2.putText(foreground_pair, time_stamp_display,
                                (18, 28), 2, 1, (0, 0, 0), 2)
                    cv2.putText(foreground_pair, time_stamp_display,
                                (18, 28), 2, 1, (255, 255, 255), 1)
                    if self._displayOrSave == 'display':
                        ...
                        cv2.imwrite(self._framePath + time_stamp_save +
                                    '_foreground.jpg', foreground_pair)
                    # Display and save frame.
                    elif self._displayOrSave == 'both':
                        cv2.imshow('foreground', foreground_pair)
                        cv2.imwrite(self._framePath + time_stamp_save +
                ...
                if self._isPlotDetected & (frameL is not None) & (frameR is not None):
                    ...
                    if self._isPlotEncloseCircle:
                        ...
                        if all(x is not None for x in [centerL_x_enclose, centerL_y_enclose, radiusL_enclose]):
                            ...
                            cv2.circle(
                                output_l, (centerL_x_enclose, centerL_y_enclose), radiusL_enclose, (255, 255, 255), 2)
                            cv2.rectangle(output_l, (centerL_x_enclose - 2, centerL_y_enclose - 2),
                                          (centerL_x_enclose + 2, centerL_y_enclose + 2), (255, 255, 255), -1)
                        if all(x is not None for x in [centerR_x_enclose, centerR_y_enclose, radiusR_enclose]):
                            ...
                            cv2.circle(
                                output_r, (centerR_x_enclose, centerR_y_enclose), radiusR_enclose, (255, 255, 255), 2)
                            cv2.rectangle(output_r, (centerR_x_enclose - 2, centerR_y_enclose - 2),
                                          (centerR_x_enclose + 2, centerR_y_enclose + 2), (255, 255, 255), -1)
                    if self._isPlotFittedCircle:
                        ...
                        if all(x is not None for x in [centerL_x_fit, centerL_y_fit, radiusL_fit]):
                            ...
                            # circle.
                            cv2.circle(
                                output_l, (centerL_x_fit, centerL_y_fit), radiusL_fit, (0, 255, 0), 2)
                            # center.
                            cv2.rectangle(output_l, (centerL_x_fit - 2, centerL_y_fit - 2),
                                          (centerL_x_fit + 2, centerL_y_fit + 2), (0, 255, 0), -1)
                        # Plot right frame.
                        if all(x is not None for x in [centerR_x_fit, centerR_y_fit, radiusR_fit]):
                            ...
                            cv2.circle(
                                output_r, (centerR_x_fit, centerR_y_fit), radiusR_fit, (0, 255, 0), 2)
                            cv2.rectangle(output_r, (centerR_x_fit - 2, centerR_y_fit - 2),
                                          (centerR_x_fit + 2, centerR_y_fit + 2), (0, 255, 0), -1)
                    detected_frame_pair = np.hstack((output_l, output_r))
                    cv2.putText(detected_frame_pair, time_stamp_display,
                                (18, 28), 2, 1, (0, 0, 0), 2)
                    cv2.putText(detected_frame_pair, time_stamp_display,
                                (18, 28), 2, 1, (255, 255, 255), 1)
                    if self._displayOrSave == 'display':
                        ...
                        cv2.imwrite(self._framePath + time_stamp_save +
                                    '_detected.jpg', detected_frame_pair)
                    # Display and save frame.
                    elif self._displayOrSave == 'both':
                        cv2.imshow('detected', detected_frame_pair)
                        cv2.imwrite(self._framePath + time_stamp_save +
                                    '_detected.jpg', detected_frame_pair)
                frame_counts += 1
                # This serves all cv2.imshow(). cv2.imshow() only work with cv2.waitKey().
                cv2.waitKey(1)
...
class Balloon_destroyer:
    ...
    def __init__(self, vehicle, cameraL, cameraR, color_range_pair, time_out, max_altitude, max_radius, framePath):
        ...
        self._detect_balloon = Thread_detect_balloon(
            cameraL, cameraR, color_range_pair)
    ...
    def get_vision_distance(self, x_diff):
        ...
        visionDistance = disparity2distance(
            x_diff, baseLine=0.14, focalLength=4.35e-3)
    ...
    def approach_balloon_sonar(self):
        ...
        while ((self.get_sonar_distance() > 2) & (self.get_distance_to_home() < self._max_radius) & rtv):
            ...
            dcf.send_body_frame_velocity_once(
                self._vehicle, 0.5, 0, 0)  # Fly forward at 0.5 m/s
    ...
    def approach_balloon_old(self):  # By vision.
        ...
        while True:
            ...
            ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
             self._R_r)) = self._detect_balloon.locate_balloon_enc()
            if (self._L_r is not None) & (self._R_r is not None):
                ...
                if ((r_mean < 45) & (self.get_distance_to_home() < self._max_radius) & rtv):
                    # Every time approaching balloon, adjust the balloon position in view field.
                    # rtv is False when balloon is not detected or lost.
                    rtv = self.make_balloon_in_view_center()
                    # Vx: forward from the current vehicle position
                    # Vy: to the right
                    # Vz: down (forward, right and down are "positive" values).
                    ...
                    dcf.air_break(self._vehicle)
                    print(
                        '{} - Function approach_balloon() is finished.\n'.format(time.ctime()))
                    return rtv
            else:
                if (counter > 0):
                    time.sleep(0.1)
                    ...
                    dcf.air_break(self._vehicle)
                    print(
                        '{} - Function approach_balloon() is finished. Balloon lost.\n'.format(time.ctime()))
                    return False
    ...
    def approach_balloon(self):  # By vision.
        ...
        while True:
            ...
            if (self.get_distance_to_home() > self._max_radius):
                ...
                dcf.air_break(self._vehicle)
                print(
                    '{} - Function approach_balloon() is stopped. Drone exceeds distance limitation.\n'.format(time.ctime()))
                return False
            # Get detecting result.
            ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
             self._R_r)) = self._detect_balloon.locate_balloon_enc()
            # Balloon is detected.
            if (self._L_r is not None) & (self._R_r is not None):
                # If the radius of the balloon is less than 45 pixels.
                if ((self._L_r + self._R_r)//2 < 45):
                    # Every time approaching balloon, adjust the balloon position in view field.
                    # Return value is False if self.make_balloon_in_view_center() fails.
                    if (self.make_balloon_in_view_center()):
                        ...
                        dcf.air_break(self._vehicle)
                        print(
                            '{} - Function approach_balloon() is stopped. Balloon is lost while making it in view field center.')
                        return False
                else:  # Drone is getting close enough to balloon.
                    # Stop moving.
                    dcf.air_break(self._vehicle)
                    print(
                        '{} - Function approach_balloon() is finished. Drone is close to the balloon.'.format(time.ctime()))
                    print('{} - Sonar distance = {}.\n'.format(time.ctime(),
                          self.get_sonar_distance()))
                    return True
                ...
                if (counter > 0):  # If not exceed the maximum trial times.
                    ...
                    dcf.air_break(self._vehicle)
                    print(
                        '{} - Function approach_balloon() is finished. Balloon is lost when drone is approaching to it.\n'.format(time.ctime()))
                    return False
    ...
    def make_balloon_in_view_center_x(self):
        ...
        ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
         self._R_r)) = self._detect_balloon.locate_balloon()
        # If balloon is detected.
        # Detected.
        if all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)):
            # If balloon is already in 70 pixel box, do nothing.
            if (((self._L_x + self._R_x)//2 > 205) and ((self._L_x + self._R_x)//2 < 275)):
                dcf.air_break(self._vehicle)
                print(
                    '{} - Do not need adjust x! Balloon is already in the 70 pixel center box.\n'.format(time.ctime()))
                return True
                ...
                for temp in xrange(150):  # 150*0.1s = 15s at most.
                    ...
                    if ((time.time()-self._start_time) >= self._time_out):
                        dcf.air_break(self._vehicle)
                        print(
                            '{} - Time out! Function make_balloon_in_view_center_x() is stopped.\n'.format(time.ctime()))
                        return False
                    # Get latest balloon location.
                    ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
                     self._R_r)) = self._detect_balloon.locate_balloon()
                    ...
                    if all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)):
                        # Adjust yaw.
                        # In left view field, drone turn left.
                        if ((self._L_x + self._R_x)//2 < 225):
                            # Rotate drone 1 degree to the left.
                            print(
                                '{} - Rotate vehicle 1 degree to the left.\n'.format(time.ctime()))
                            # turn_direction -1 : counter clockwise
                            dcf.send_body_frame_yaw_once(self._vehicle, 1, -1)
                            # Upon testing, the turning speed is 30 degree/second.
                            time.sleep(0.1)
                        # In right view field, drone turn right.
                        elif ((self._L_x + self._R_x)//2 > 255):
                            # Rotate drone 1 degree to the right.
                            print(
                                '{} - Rotate vehicle 1 degree to the right.\n'.format(time.ctime()))
                            ...
                            dcf.air_break(self._vehicle)
                            print(
                                '{} - Done! Function make_balloon_in_view_center_x() is finished.\n'.format(time.ctime()))
                            return True
                    else:  # Balloon lost.
                        dcf.air_break(self._vehicle)
                        self._isFoundBalloon = False  # Balloon lost.
                        print(
                            '{} - Balloon lost! Function make_balloon_in_view_center_y() is stopped.\n'.format(time.ctime()))
                        return False
        else:  # Balloon is not detected.
            dcf.air_break(self._vehicle)
            self._isFoundBalloon = False  # Balloon lost.
            print(
                '{} - Balloon lost! Function make_balloon_in_view_center_x() is stopped.\n'.format(time.ctime()))
            return False
    ...
    def make_balloon_in_view_center_y(self):
        ...
        if all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)):
            ...
            if (((self._L_y + self._R_y)//2 > 100) and ((self._L_y + self._R_y)//2 < 170)):
                dcf.air_break(self._vehicle)
                print(
                    '{} - Do not need adjust y! Balloon is already in the 70 pixel center box.\n'.format(time.ctime()))
                return True
            else:  # If balloon is outside the 70 pixel center box.
                # Adjust vehicle to make balloon appear in 30 pixel center box.
                for temp in xrange(150):  # 150*0.1s = 15s at most.
                    ...
                    if ((time.time()-self._start_time) >= self._time_out):
                        dcf.air_break(self._vehicle)
                        print(
                            '{} - Time out! Function make_balloon_in_view_center_y() is stopped.\n'.format(time.ctime()))
                        return False
                    # Get latest balloon location.
                    ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
                     self._R_r)) = self._detect_balloon.locate_balloon()
                    ...
                    if all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)):
                        ...
                        if (y_mean < 120):  # Above center, drone moves up.
                            ...
                            # Vz: down (forward, right and down are "positive" values).
                            print(
                                '{} - y_mean<120, move vehicle up.\n'.format(time.ctime()))
                            dcf.send_body_frame_velocity_once(
                                self._vehicle, 0, 0, -0.5)  # Fly up at 0.5 m/s
                            # Sleep 0.1 second. 0.5 m/s * 0.1 s = 5cm
                            time.sleep(0.1)
                        elif (y_mean > 150):  # Below center, drone moves down.
                            print(
                                '{} - y_mean>150, move vehicle down.\n'.format(time.ctime()))
                            dcf.send_body_frame_velocity_once(
                                self._vehicle, 0, 0, 0.5)  # Fly down at 0.5 m/s
                            # Sleep 0.1 second. 0.5 m/s * 0.1 s = 5cm
                            time.sleep(0.1)
                        else:  # In center
                            dcf.air_break(self._vehicle)
                            print(
                                '{} - Done! 120<y_mean<150. Function make_balloon_in_view_center_y() is finished.\n'.format(time.ctime()))
                            return True
                    else:  # Balloon lost.
                        dcf.air_break(self._vehicle)
                        self._isFoundBalloon = False  # Balloon lost.
    ...
    def rotate_to_search_balloon(self):
        ...
        while True:
            ...
            if ((time.time()-self._start_time) >= self._time_out):
                dcf.air_break(self._vehicle)
                print(
                    '{} - Time out! Function rotate_to_search_balloon() is stopped.\n'.format(time.ctime()))
                return False
            # Get detecting result.
            ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
             self._R_r)) = self._detect_balloon.locate_balloon()
            ...
            # Not detected.
            if not all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)):
                # turn_direction 1 : clockwise
                dcf.send_body_frame_yaw_once(self._vehicle, 1, 1)
                # Upon testing, the turning speed is 30 degree/second.
                time.sleep(0.1)
                ...
                if (degree_to_turn <= 0):
                    ...
                    dcf.air_break(self._vehicle)
                    print(
                        '{} - 360 degree rotation is done! function rotate_to_search_balloon() is finished.\n'.format(time.ctime()))
                    return False
            else:  # Detected.
                dcf.air_break(self._vehicle)
                ...
                print(
                    '{} - Balloon is found! function rotate_to_search_balloon() is finished.\n'.format(time.ctime()))
                return True

    # This is the main function in this class.
    def find_and_destroy_balloon(self):
        ...
        # Cameras start capturing frames and save results to deque.
        self._detect_balloon.start()
        # Start video recording.
        print('{} - Start video recording...\n'.format(time.ctime()))
        self._detect_balloon.start_video_recording(
            isPlotOriginalFrame=False, displayOrSave='save', framePath=self._framePath)
        ...
        while True:
            ...
            if ((time.time()-self._start_time) >= self._time_out):
                dcf.air_break(self._vehicle)
                print(
                    '{} - Time out! Function find_and_destroy_balloon() is stopped.\n'.format(time.ctime()))
                print(
                    '{} - Mission Failed! Balloon is not found!.\n'.format(time.ctime()))
                time.sleep(1)
            ...
            if (vehicle.location.global_relative_frame.alt < self._max_altitude):
                ...
                for temp in xrange(4):  # Fly up 20 cm.
                    # Fly up at 0.5 m/s. Negative z is up.
                    dcf.send_body_frame_velocity_once(
                        self._vehicle, 0, 0, -0.5)
                    # Sleep 0.1 second. 0.5 m/s * 4 * 0.1 s = 20 cm
                    time.sleep(0.1)
                ...
                ((self._L_x, self._L_y, self._L_r), (self._R_x, self._R_y,
                 self._R_r)) = self._detect_balloon.locate_balloon()
                # If detected.
                if all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r)) and ((self._L_r + self._R_r)/2.0 >= 5):
                    ...
                    # This function calls make_balloon_in_view_center() first.
                    self.approach_balloon()
                    # At last step, use finer adjustment.
                    rtv = self.make_balloon_in_view_center_finer()
                    ...
                    if rtv:  # If balloon is still in view field after approaching.
                        ...
                        if (all(item is not None for item in (self._L_x, self._L_y, self._L_r, self._R_x, self._R_y, self._R_r))) and ((self._L_r + self._R_r)/2.0 >= 30):
                            ...
                            # Send command 10 times, total time is 1 second.
                            for i_temp in xrange(10):
                                # Vx: forward from the current vehicle position
                                # Vy: to the right
                                # Vz: down (forward, right and down are "positive" values).
                                dcf.send_body_frame_velocity_once(
                                    self._vehicle, 5, 0, 0)  # Fly forward at 5 m/s
                                time.sleep(0.1)
                            # After hitting, change state and air break.
                            self._isMissionCompleted = True
                            dcf.air_break(self._vehicle)
                            print(
                                '{} - Mission Completed! Balloon is destroyed!.\n'.format(time.ctime()))
                            # Stop video recording.
                            self._detect_balloon.stop_video_recording()
                            # Stop continuously detection.
                            self._detect_balloon.stop()
                            ...
                            print(
                                '{} - Mission Failed! The detected object is not balloon!.\n'.format(time.ctime()))
                            time.sleep(1)
                            # Stop video recording.
                            self._detect_balloon.stop_video_recording()
                            # Stop continuously detection.
                        ...
                        print(
                            '{} - Mission Failed! Balloon is lost during approaching!.\n'.format(time.ctime()))
                        time.sleep(1)
                        # Stop video recording.
                        self._detect_balloon.stop_video_recording()
                        # Stop continuously detection.
                ...
                while (count_move_down > 0):
                    # If balloon is found during rotating.
                    if (self.rotate_to_search_balloon()):
                        ...
                        # Move vehicle down 0.5 meter to search again.
                        for temp in xrange(10):
                            dcf.send_body_frame_velocity_once(
                                self._vehicle, 0, 0, 0.5)  # Fly down at 0.5 m/s
                            # Sleep 0.1 second. 0.5 m/s * 10 * 0.1 s = 50 cm
                            time.sleep(0.1)
                        dcf.air_break(self._vehicle)
                        count_move_down -= 1

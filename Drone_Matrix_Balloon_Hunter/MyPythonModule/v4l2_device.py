...
# This import the module from user folder. Python wrapper v4l2 API.
from MyPythonModule import v4l2
...
class Camera():
    ...
    def open(self):
        if self.fd is None:
            ...
            print('Error: Can not open device "{}", because it is already opened!'.format(
                self.name))
    ...
    def close(self):
        if self.fd is not None:
            ...
            print('Error: Can not close device "{}", because it is not opened!'.format(
                self.name))
    ...
    def get_supported_format(self):
        ...
        while (0 == xioctl(self.fd, v4l2.VIDIOC_ENUM_FMT, fmtdesc)):
            format_dict[str(fmtdesc.index)] = {'pixelformat': v4l2.v4l2_fourcc2str(
                fmtdesc.pixelformat), 'description': fmtdesc.description}
            fmtdesc.index += 1
        return format_dict
    ...
    def set_video_format(self, frame_width, frame_height, pixelformat_str):
        ...
        pixelformat = v4l2.v4l2_fourcc(
            pixelformat_str[0], pixelformat_str[1], pixelformat_str[2], pixelformat_str[3])
        ...
        if (-1 == xioctl(self.fd, v4l2.VIDIOC_S_FMT, fmt)):
            ...
            print('pixelformat : {}\n'.format(
                v4l2.v4l2_fourcc2str(fmt.fmt.pix.pixelformat)))
            return 0
    ...
    def get_control_parameter(self, CID):
        ...
        queryctrl.id = CID
        control.id = CID
        rtv_VIDIOC_QUERYCTRL = xioctl(
            self.fd, v4l2.VIDIOC_QUERYCTRL, queryctrl)
        ...
        if (-1 == rtv_VIDIOC_QUERYCTRL):
            ...
            print('flags         = {}'.format(
                v4l2.v4l2_CTRL_FLAG_dict[queryctrl.flags]))
            print('type          = {}'.format(
                v4l2.v4l2_ctrl_type_dict[queryctrl.type]))
    ...
    def set_control_parameter(self, CID, control_value):
        ...
        if (-1 == rtv_VIDIOC_QUERYCTRL):
            print('Before setting, failed to query control parameter for device "{}".'.format(
                self.name))
            print('CID    = {}'.format(v4l2.v4l2_CID_dict[CID]))
            # print('\n')
            return -1
        elif (-1 == rtv_VIDIOC_G_CTRL):
            print('Before setting, failed to get control parameter for device "{}"'.format(
                self.name))
            print('CID    = {}'.format(v4l2.v4l2_CID_dict[CID]))
            # print('\n')
            return -1
            ...
            rtv_VIDIOC_S_CTRL = xioctl(
                self.fd, v4l2.VIDIOC_S_CTRL, new_setting)
            if (-1 == rtv_VIDIOC_S_CTRL):
                print(
                    'Failed to set control parameter for device "{}".'.format(self.name))
                print('CID    = {}'.format(v4l2.v4l2_CID_dict[CID]))
                print('\n')
                return -1
            else:
                print(
                    'Set control parameter for device "{}" successfully.'.format(self.name))
                # Query new setting.
                rtv_VIDIOC_QUERYCTRL = xioctl(
                    self.fd, v4l2.VIDIOC_QUERYCTRL, queryctrl)
                rtv_VIDIOC_G_CTRL = xioctl(
                    self.fd, v4l2.VIDIOC_G_CTRL, new_setting)
                if (-1 == rtv_VIDIOC_QUERYCTRL):
                    print('After setting, failed to query control parameter for device "{}".'.format(
                        self.name))
                    print('CID    = {}'.format(v4l2.v4l2_CID_dict[CID]))
                    # print('\n')
                    return -1
                elif (-1 == rtv_VIDIOC_G_CTRL):
                    print('After setting, failed to get control parameter for device "{}".'.format(
                        self.name))
                    print('CID    = {}'.format(v4l2.v4l2_CID_dict[CID]))
                    # print('\n')
                    return -1
                    ...
                    print('step          = {}'.format(queryctrl.step))
                    print('default_value = {}'.format(queryctrl.default_value))
                    print('current_value = {} (old value = {})'.format(
    ...
    def get_supported_frame_size_and_fps(self):
        ...
        # Must fill .index and .type before use.
        fmtdesc = v4l2.v4l2_fmtdesc()
        # Must fill .index and .pixel_format before use.
        frmsize = v4l2.v4l2_frmsizeenum()
        # Must fill .index, .pixel_format, .width, and .height before use.
        frmival = v4l2.v4l2_frmivalenum()
        fmtdesc.index = 0
        fmtdesc.type = v4l2.V4L2_BUF_TYPE_VIDEO_CAPTURE
        while (0 == xioctl(self.fd, v4l2.VIDIOC_ENUM_FMT, fmtdesc)):
            print('Pixel Format : {}'.format(
                v4l2.v4l2_fourcc2str(fmtdesc.pixelformat)))
            frmsize.index = 0
            ...
            while ((0 == xioctl(self.fd, v4l2.VIDIOC_ENUM_FRAMESIZES, frmsize))):
                if (frmsize.type == v4l2.V4L2_FRMSIZE_TYPE_DISCRETE):
                    ...
                    while (0 == xioctl(self.fd, v4l2.VIDIOC_ENUM_FRAMEINTERVALS, frmival)):
                        ...
                        print('    discrete {: >4}x{: <4} ({: >3} fps)'.format(
                    ...
                    print('    stepwise {}x{}'.format(
                        frmsize.stepwise.min_width, frmsize.stepwise.min_height))
                    print('    min_width   = {}'.format(
                        frmsize.stepwise.min_width))
                    print('    max_width   = {}'.format(
                        frmsize.stepwise.max_width))
                    print('    step_width  = {}'.format(
                        frmsize.stepwise.step_width))
                    print('    min_height  = {}'.format(
                        frmsize.stepwise.min_height))
                    print('    max_height  = {}'.format(
                        frmsize.stepwise.max_height))
                    print('    step_height = {}'.format(
    ...
    def get_device_capability(self):
        ...
        for key in v4l2.v4l2_capabilities_dict:
            if (capabilities & key):
                print('               {}'.format(
                    v4l2.v4l2_capabilities_dict[key]))
    ...
    def init_device(self, resolution=(480, 270), pixel_format='MJPG', exposure='AUTO', white_balance='AUTO'):
        ...
        if (exposure == 'AUTO'):
            ...
            self.set_control_parameter(v4l2.V4L2_CID_EXPOSURE_AUTO, 3)
            # Do not vary frame rate according to exposure.
            self.set_control_parameter(v4l2.V4L2_CID_EXPOSURE_AUTO_PRIORITY, 0)
            ...
            if ((exposure < 1) or (exposure > 5000)):
                ...
                # 1 = Manual Mode, 3 = Aperture Priority Mode
                self.set_control_parameter(v4l2.V4L2_CID_EXPOSURE_AUTO, 1)
                # Default 177.
                self.set_control_parameter(
                    v4l2.V4L2_CID_EXPOSURE_ABSOLUTE, exposure)
        else:
        ...
        if (white_balance == 'AUTO'):
            # 1 = auto, 0 = manual.
            self.set_control_parameter(v4l2.V4L2_CID_AUTO_WHITE_BALANCE, 1)
        # white balance temperature range = (2800 ~ 6500), 4600 by default.
        elif (isinstance(white_balance, (int, long))):
            if ((white_balance < 2800) or (white_balance > 6500)):
                print(
                    'Range Error: white balance temperature range is from 2800 to 6500.')
                return -1
            else:
                # 1 = auto, 0 = manual.
                self.set_control_parameter(v4l2.V4L2_CID_AUTO_WHITE_BALANCE, 0)
                self.set_control_parameter(
                    v4l2.V4L2_CID_WHITE_BALANCE_TEMPERATURE, white_balance)
        else:
            print(
                'Value Error: please specify "AUTO" or white balance temperature value (int).')
            return -1
        return 0
    ...
    def init_mmap(self):
        ...
        for i in xrange(self._buffer_count):
            ...
            # self._buffer.index will automatically increased by 1.
            if (-1 == xioctl(self.fd, v4l2.VIDIOC_QUERYBUF, self._buffer)):
    ...
    def stream_on(self):
        for i in xrange(self._buffer_count):
            self._buffer.index = i
            # Queue buffer won't increase buffer index.
    ...
    def retrieve(self):
        ...
        # Dequeue buffer will increase buffer.index by 1.
        if (-1 == xioctl(self.fd, v4l2.VIDIOC_DQBUF, self._buffer)):
            print('retrieve() failed. Can not dequeue buffers.\n')
            return None
        try:
            raw_img = np.asarray(bytearray(
                self._mmap_dict[self._buffer.index].read(self._buffer_size)), dtype=np.uint8)
            # Rollback buffer pointer.
            self._mmap_dict[self._buffer.index].seek(0)
            return raw_img
        except Exception:
            return None
    ...
    def decode_MJPG_downsample(self, raw_img, (width, height), color=True):
        try:
            ...
            # 8.04901123047 ms to load 1920x1080.
